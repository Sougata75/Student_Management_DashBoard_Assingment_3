...
import {
  columnName,
  userRoleSelection,
} from "../../services/json/studentManagement.table";
  ...
  useEffect(() => {
    if (userContext.isEdit && userContext.editMode.length > 0) {
      ...
      resetEdit({
        ...
        role: student.role,
        enrolled_courses:
          student.enrolled_courses?.map((courses: any) =>
            typeof courses === "string" ? courses : courses.$id,
          ) || [],
  ...
  return (
    <>
      <Box className="w-full md:w-[98%] h-[90.5vh] md:h-full bg-white md:shadow-[0px_0px_20px] md:shadow-gray-500 md:rounded-md px-2 pt-2 md:p-5 flex flex-col justify-between">
        <Box className="w-full h-[10%]">
          <Box className="w-full h-full bg-slate-300 shadow-xl shadow-gray-400 rounded-md flex justify-between items-center p-4">
            ...
            <Box
              component={"form"}
              onSubmit={handleSubmit(userContext.searchFunc)}
            ...
            >
              ...
              <Button
                type="submit"
                sx={{ bgcolor: "#2563eb", color: "black" }}
                className="h-[39px] hover:bg-blue-700 hover:text-white"
              >
                <Search color="#ffffff" />
              </Button>
            </Box>
        ...
        <Box className="w-full h-[86%] bg-slate-300 p-2 md:p-5 shadow-xl shadow-gray-400 rounded-md flex flex-wrap items-center md:justify-center">
          <Box className="w-full h-[10%] py-3 hidden md:flex flex-wrap justify-between md:gap-2">
            {columnName.map((col) => (
              <Box
                key={col}
                className="w-[16%] flex justify-center bg-slate-100 items-center rounded-sm md:rounded-md"
              >
                <Typography
                  variant="h4"
                  sx={{ fontWeight: "600" }}
                  className="!text-[10px] md:!text-[16px] text-slate-600"
                >
                  {col}
                </Typography>
              </Box>
          ...
          <Box
            component={"form"}
            className="w-full h-[10%]  bg-slate-200 px-2 rounded-md  md:hidden flex flex-wrap justify-between items-center"
          >
            <TextField
              size="small"
              placeholder="Search by email or contact number"
          ...
          {userContext?.isLoading ? (
            ...
            <Box className="w-full h-[85%] overflow-y-scroll  flex flex-wrap justify-center  noBar">
              {userContext?.searchResult.map((user) => (
                <Box
                  key={user.$id}
                  className="w-full  h-min flex flex-wrap justify-between items-center border-b border-gray-400 rounded-md bg-gray-200 hover:bg-gray-50 transition-colors duration-300 py-[14.2px] px-5"
                >
                  <Box className="w-1/6 hidden md:block">
                    <Typography
                      variant="h4"
                      className="!text-[16px] text-slate-800"
                    >
                      {user.name}
                    </Typography>
                  </Box>
                  ...
                  <Box className="w-1/6 hidden md:flex justify-center">
                    <Button
                      onClick={() => {
                        (userContext.setisEdit(true),
                          userContext.seteditMode([user]));
                      }}
                    >
                      <EditIcon />
                    </Button>
                  ...
                  <Box className="w-1/6 hidden md:flex justify-center md:pl-12">
                    <Button
                      onClick={() => userContext.handleDelete(user.$id)}
                      variant="contained"
                      size="medium"
                      color="error"
                    >
      ...
      <Dialog open={userContext.isEdit === true}>
        ...
        <DialogContent
          sx={{ width: "450px" }}
          className="!w-[310px] md:!w-[450px]"
        >
          <Box
            onSubmit={handleSubmitEdit(userContext.updateUser)}
            component={"form"}
          ...
          >
            ...
            <FormControl fullWidth>
              <InputLabel>Assing Courses</InputLabel>
              <Controller
            ...
            <Box className="w-full flex flex-wrap justify-between items-center">
              ...
              <Button
                ...
                onClick={() =>
                  setValue("isBlocked", !currentStatus, { shouldDirty: true })
                }
              >
                {currentStatus ? "Blocked" : "Active"}
              </Button>
            </Box>
            ...
            <Button
              variant="contained"
              onClick={() => userContext.setisEdit(false)}
              className="w-full"
            >
              Close
            </Button>
